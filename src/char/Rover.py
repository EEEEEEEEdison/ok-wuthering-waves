...
class Rover(BaseChar):
    ...
    def init(self):
        ...
        if hasattr(self.task, '_ensure_ring_index'):
            self.task._ensure_ring_index(refresh=True)
        if self.ring_index != previous_form:
            self._bind_form_logger()
            self.logger.info(
                f'rover: form-corrected previous={previous_form} current={self.ring_index}'
            )
            if hasattr(self.task, 'refresh_rover_form_context'):
                self.task.refresh_rover_form_context(self)
            elif hasattr(self.task, 'refresh_chars_display'):
                self.task.refresh_chars_display()
